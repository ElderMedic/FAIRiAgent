...
def metadata_table_profiles_for_document_datasets(
    workspace: Mapping[str, Any],
    dataset_mentions: Iterable[Any],
    *,
    max_examples: int = 8,
    max_candidates: int = 32,
) -> List[Dict[str, Any]]:
    """Profile exact table subsets named by parsed dataset evidence.

    Dataset mentions are already source-derived semantic evidence.  Matching
    their exact identifier tokens against table cell values gives the structure
    planner a compact focal-population view before it has drafted a record plan.
    This is deliberately value-driven: no accession prefix or study-specific
    identifier pattern is assumed, and code still leaves the completeness and
    ISA grouping decisions to the planner.
    """
        ...
        for table in source.get("tables") or []:
            ...
            for column in columns:
                ...
                for value in sorted(values):
                    mentions = mention_tokens.get(value.casefold())
                    if not mentions:
                        continue
                    selected = [
                        row for row in records if _text(row.get(column)) == value
                    ]
                    ...
                    if key in seen:
                        continue
                    seen.add(key)
                    profile = _metadata_table_profile(
                        source_id=source.get("source_id"),
                        table_name=table.get("name"),
                        source_path=source.get("path"),
                        header_index=header_index,
                        ...
                        applied_filters=[{"column": column, "value": value}],
                    )
                    profile["document_dataset_mentions"] = list(dict.fromkeys(mentions))
                    profile["candidate_filter_requires_planner_confirmation"] = True
                    mention_text = " ".join(profile["document_dataset_mentions"]).lower()
                    focal_cues = (
                        "this study",
                        "this work",
                        "our study",
                    ...
                    focal_score = sum(
                        2 for cue in focal_cues if cue in mention_text
                    ) - sum(2 for cue in contextual_cues if cue in mention_text)
                    profile["focal_evidence_score"] = focal_score
                    candidates.append(profile)
    candidates.sort(
        key=lambda item: (
            -int(item.get("focal_evidence_score") or 0),
            _text(item.get("source_id")),
            _text(item.get("table_name")),
            _text((item.get("applied_filters") or [{}])[0].get("value")),
        )
    )
