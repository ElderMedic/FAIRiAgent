...
"""
    ...
    logger.info(f"📝 Created temp config: {temp_env_path}")
...
def apply_user_config(
    ...
    # 更新环境变量（用于 LangSmith 等）
    if langsmith_key:
        os.environ["LANGSMITH_API_KEY"] = langsmith_key

    logger.info(
        f"⚙️ Applied config: {llm_provider}/{llm_model}, "
...
async def process_document_stream(
    ...
    try:
        ...
        output_text = "🚀 **Starting FAIRifier Workflow**\n\n"
        output_text += f"⚙️ **Configuration**:\n"
        ...
        output_text += f"📋 **Project ID:** `{project_id}`\n"
        output_text += f"📁 **File:** {Path(file_path).name}\n"
        output_text += f"📝 **Config File:** `{temp_env}`\n\n"
        yield output_text
        await asyncio.sleep(0.3)
        ...
        try:
            ...
            async for event in workflow.workflow.astream(
                ...
                if isinstance(event, tuple) and len(event) >= 2:
                    ...
                    # 格式化节点信息
                    output_text += f"### 🔷 {node_name}\n"
                    output_text += f"⏱️ *Time: {elapsed:.1f}s | Step: {step_count}*\n\n"
                    ...
                    if isinstance(node_output, dict):
                        ...
                        if conv.get("success"):
                            output_text += "📄 **Document Conversion**\n"
                            output_text += f"  - ✅ Success | Method: {conv.get('method', 'N/A')}\n"
                            ...
                            output_text += f"❌ **Conversion Error**: {conv['error']}\n\n"

                        # 知识检索
                        knowledge = node_output.get("retrieved_knowledge", [])
                        if knowledge:
                            output_text += f"📚 **Knowledge Retrieved**: {len(knowledge)} items\n"
                        ...
                        if fields:
                            output_text += f"🏷️ **Metadata Generated**: {len(fields)} fields\n"
                            # 按类别统计
                            categories = {}
                            for field in fields:
                        ...
                        if history:
                            ...
                            if critic:
                                ...
                                output_text += f"  - 🤖 **Critic**: {decision} (score: {score:.2f})\n"

                                strengths = critic.get("strengths", [])
                                if strengths:
                                    output_text += f"  - ✅ Strengths: {', '.join(strengths[:2])}\n"

                                weaknesses = critic.get("weaknesses", [])
                                if weaknesses:
                                    output_text += f"  - ⚠️ Weaknesses: {', '.join(weaknesses[:2])}\n"

                                suggestions = critic.get("suggestions", [])
                                if suggestions:
                                    output_text += f"  - 💡 Suggestions: {suggestions[0]}\n"
                        ...
                        if conf.get("overall"):
                            ...
                            output_text += f"🎯 **Confidence**: {overall:.1%}"
                            if "critic" in conf:
...
async def process_document_full(
    ...
    try:
        ...
        # 运行 workflow（使用更新后的 config）
        workflow = get_workflow()
        result = await workflow.run(file_path, project_id, output_dir=str(output_path))
        
        status = result.get("status", "unknown")
        
        status_msg = f"✅ Processing completed! Status: {status.upper()}"
        if result.get("needs_human_review"):
            status_msg += " (⚠️ Needs review)"
        
        # 返回: (完整结果JSON, 状态消息, 输出目录)
...
def format_execution_history(result: Dict) -> str:
    ...
    for i, exec_record in enumerate(history, 1):
        ...
        success = "✅" if exec_record.get("success", False) else "❌"
...
def format_metadata_fields(result: Dict) -> Tuple[str, str]:
    ...
    # Markdown
    md = f"## 🏷️ Metadata Fields ({total} total, {required} required)\n\n"
    ...
    for sheet_name in ["investigation", "study", "assay", "sample", "observationunit", "unassigned"]:
        ...
        for field in sheet_fields[:10]:  # 只显示前10个
...
def create_gradio_interface():
    ...
    with gr.Blocks() as demo:
        ...
        # 标题
        gr.Markdown("""
        # 🧬 FAIRifier - FAIR Metadata Generator
        ...
        with gr.Tabs():
            ...
            with gr.Tab("📄 Upload & Process", id="upload"):
            ...
            with gr.Tab("📊 Results Summary", id="results"):
                ...
                show_json_btn = gr.Button("📋 Show Full JSON")
            ...
            with gr.Tab("🔍 Execution History", id="history"):
            ...
            with gr.Tab("🎯 Confidence Details", id="confidence"):
            ...
            with gr.Tab("🏷️ Metadata Fields", id="metadata"):
...
def main():
    ...
    logger.info(f"🔧 MinerU: {'Enabled' if config.mineru_enabled else 'Disabled'}")
