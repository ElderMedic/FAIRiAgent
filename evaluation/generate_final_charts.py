...
from evaluation.scripts.compare_values_against_gt import load_gt_sheets, load_run_sheets, align_rows, combined_score, _normalise_field_name
...
def get_run_dirs(condition, doc):
    if condition == 'full_pipeline':
        # the best performing model config or average across them
        pattern = str(RUNS_DIR / condition / "*" / doc / "run_1")
    else:
...
for doc in DOCS:
    ...
    gt_total_fields = sum(len(row) for rows in gt_sheets.values() for row in rows)
    ...
    for cond_key, cond_name in CONDITIONS.items():
        ...
        for run_dir in run_dirs:
            ...
            pred_total_fields = sum(len(row) for rows in pred_sheets.values() for row in rows)
            if pred_total_fields == 0:
                results.append({
            ...
            for sheet_name, gt_rows in gt_sheets.items():
                from evaluation.scripts.compare_values_against_gt import evaluate_sheet
...
plt.title('Exp 1: Hierarchical Metadata Quality (Official Semantic Evaluator)', pad=20, fontweight='bold')
plt.ylabel('Score')
plt.xlabel('')
plt.ylim(0, 1.05)
...
plt.figtext(0.5, 0.03, "* Results averaged across 8 benchmark documents (Tier A&B).\nHierarchical-F1 uses Hungarian Row Alignment. Value Accuracy uses Semantic Similarity (all-MiniLM-L6-v2).", ha="center", fontsize=11, style='italic', color='#555555')
