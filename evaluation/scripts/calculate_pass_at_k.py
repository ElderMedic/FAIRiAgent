...
class SuccessCriteria:
    ...
    # Layer 1 recall (optional). min_f1_score / min_precision are legacy
    # diagnostics -- defaults keep them at 0.
...
def is_successful(eval_result: Dict[str, Any], criteria: SuccessCriteria) -> Tuple[bool, List[str]]:
    ...
    # Correctness metrics (Layer 1: field-name coverage only, never checks
    # values -- see `value_accuracy` for real value correctness, used
    # separately since it's only populated when values-level GT exists).
